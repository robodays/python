...
class MyServoClass:
    ...
    def get_label(self):
        ...
        self.spinbox_wind[self.id_servo] = Spinbox(window, from_=self.mini, to=self.maxi, width=5,
                                                   textvariable=self.val_var[self.id_servo],
                                                   command=lambda: self.get_click())
    ...
    def get_click(self):
        ...
        self.delta = int(format(self.spinbox_wind[self.id_servo].get())) - self.begin_servo
...
def click_button_1():
    sub = Toplevel(window)
    sub.title("Code for Arduino")
    sub.geometry("820x480+100+100")
    text_scroll = Text(sub, width=100, height=5, wrap=WORD)
    text_scroll.pack(side=LEFT, fill=Y)

    scroll = Scrollbar(sub, command=text_scroll.yview)
    scroll.pack(side=LEFT, fill=Y)
    text_scroll.config(yscrollcommand=scroll.set)
    text_insert_all = "Все строчки:\n"
    text_insert_update = "\nТолько изменёные строчки: \n"
    for servo_key in range(len(servo_list)):
        string = "serv_obj[" + str(servo_list[servo_key].id_servo) + "].setSlowServo(" + str(servo_list[servo_key].delta) + ",1,delays);\n"
        text_insert_all += string
        if servo_list[servo_key].delta != 0:
            text_insert_update += string
    text_insert_all += text_insert_update
    text_scroll.insert(1.0, text_insert_all)
...
def click_button_2():
    for servo_key in range(len(servo_list)):
        servo_list[servo_key].send_arduino()
...
# Вывод кнопок
gen_code = Button(text="Code for Arduino", command=click_button_1)
gen_code.grid(column=0, row=0)
servo_start = Button(text="Start Servo", command=click_button_2)
